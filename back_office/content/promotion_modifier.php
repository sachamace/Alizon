        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                SET nom_promotion = ?,
                    ...
                    date_fin = ?,
                    est_actif = ?
                WHERE id_promotion = ? AND id_vendeur = ?
            ");
            $stmt->execute([
                $nom_promotion,
                $description,
            ...
            $stmt_delete = $pdo->prepare("
                DELETE FROM public.promotion_produit 
                WHERE id_promotion = ?
            ");
            $stmt_delete->execute([$id_promotion]);
            
            // Insérer les nouveaux produits liés
            ...
            $pdo->rollBack();
            $errors['database'] = "Erreur lors de la modification : " . $e->getMessage();
        }
    }
}
?>
...
<section class="promotion-creer-container">
    ...
    <form method="POST" class="promotion-form">
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <input type="text" 
                       id="nom_promotion" 
                       name="nom_promotion" 
                       value="<?= htmlentities($_POST['nom_promotion'] ?? $promotion['nom_promotion']) ?>"
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" 
                           id="date_debut" 
                           name="date_debut" 
                           value="<?= htmlentities($_POST['date_debut'] ?? $promotion['date_debut']) ?>"
                           required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           id="date_fin" 
                           name="date_fin" 
                           value="<?= htmlentities($_POST['date_fin'] ?? $promotion['date_fin']) ?>"
                           required>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions">
            ...
            <a href="?page=promotion&type=consulter&id=<?= $id_promotion ?>" class="btn-secondary">Annuler</a>
        </div>
    </form>
</section>
